 ...
 * 
 */
public class MyHttpClient implements java.io.Serializable {
	...
	static Logger log = Logger.getLogger(MyHttpClient.class.getName());
	...
	public MyHttpClient(int maxConPerHost, int conTimeOutMs, int soTimeOutMs,
			int maxSize) {
...
//		clientParams.setCookiePolicy(CookiePolicy.IGNORE_COOKIES);
		...
		client = new org.apache.commons.httpclient.HttpClient(clientParams,
				connectionManager);
		Protocol myhttps = new Protocol("https", new MySSLSocketFactory(), 443);//jhy err for zte internet
		...
		if (proxyHost != null && !proxyHost.equals("")) {
            ...
            final HttpHost proxyHost = new HttpHost(proxyHostName, proxyPort, 
                    new Protocol("https", new MySSLSocketFactory(), 443));
            
            client.getHostConfiguration().setHost(proxyHost);
//			client.getHostConfiguration().setProxy(proxyHost, proxyPort);//jhy
	...
	public Response get(String url, PostParameter[] params,boolean header)
            throws WeiboException {
        log("Request:");
        log("GET:" + url);
        if (null != params && params.length > 0) {
            String encodedParams = MyHttpClient.encodeParameters(params);
            if (-1 == url.indexOf("?")) {
                url += "?" + encodedParams;
            } else {
                url += "&" + encodedParams;
        ...
        return httpRequest(getmethod,header);
		...
		if (null != paging) {
			...
			if (null != params) {
				...
				if (0 != arrayPagingParams.length) {
					String encodedParams = MyHttpClient
